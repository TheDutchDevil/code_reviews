...
def split_prs_on_build_date(project, prs, equal_number = False, field_name="status_travis_date", seed = None, days_delta = 15):
    ...
    prs_before = [pr for pr in prs if pr["created_at"] < (project[field_name] - datetime.timedelta(days=days_delta))]
    prs_after = [pr for pr in prs if pr["created_at"] > (project[field_name] + datetime.timedelta(days=days_delta))]
